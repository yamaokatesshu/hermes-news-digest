...
CONFIG_PATH = PROJECT_ROOT / "config_hermes.yaml"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# File Paths
CANDIDATE_URLS_PATH = OUTPUT_DIR / "candidate_urls.txt"
PROCESSED_URLS_LOG_PATH = OUTPUT_DIR / "processed_urls.log"  # For persistence
GENERAL_LOG_PATH = OUTPUT_DIR / "discovery_general.log"

# Settings
SUMMARY_HOURS = 72  # Look for articles in the last 3 days
REQUEST_TIMEOUT = 25
POLITENESS_DELAY = 1
HEADERS = {'User-Agent': 'Hermes-News-Discoverer/1.0'}

# --- Logging Setup ---
# The format is kept simple to look clean in the GUI log viewer
logging.basicConfig(
    level=logging.INFO,
    format='%(message)s',  # UPDATED: Simpler format for the GUI
    handlers=[
...
def process_rss_source(source_config: Dict, found_urls: Set, cutoff_time: datetime):
    ...
    if not feed.entries:
        logging.warning(
            f"  - Source '{source_config.get('name')}' is not a valid RSS feed or is empty. Will attempt to crawl as HTML.")
        return False
...
def crawl_html_source(source_config: Dict, found_urls: Set, cutoff_time: datetime):
    ...
    try:
        ...
        found_count = 0
        # --- NEW: More granular logging for the user ---
        ...
        for i, article_url in enumerate(links_to_check):
            # Give a small progress update without being too noisy
            if i > 0 and i % 10 == 0:
                logging.info(f"    ({i}/{len(links_to_check)}) links checked...")

            if article_url in found_urls: continue

            # This is the most granular log, showing exactly what is being checked.
            ...
            try:
                ...
                # This is very common, so we don't log an error, we just continue.
                continue
